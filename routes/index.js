...
userSchema.plugin(uniqueValidator, {message: 'User name or Email ID already exists'});
...
var employeeSchema = mongoose.Schema({
    ...
    Oraganization_Id: [{ type: mongoose.Schema.Types.ObjectId, ref: 'Organization' }],
	status: Number
});
...
var Organization = mongoose.model('Organization', organisationSchema, 'ALB_Organization');
var Employee = mongoose.model('Employee', employeeSchema, 'ALB_Employees');
var InventoryCalender = mongoose.model('InventoryCalender', inventoryCalenderSchema, 'ALB_Inventory_Calendar');
var Workstation = mongoose.model('Workstation', workstationSchema, 'ALB_Workstation');
var Reason = mongoose.model('Reason', reasonSchema, 'ALB_Reason');
var Downtime = mongoose.model('Downtime', downtimeSchema, 'ALB_Downtime');
var ItemMaster = mongoose.model('ItemMaster', itemMasterSchema, 'ALB_Item_Master');
var AspJob = mongoose.model('AspJob', aspJobSchema, 'aspJob');
var PrdDataHr = mongoose.model('PrdDataHr', prdDataHrSchema, 'prdDataHr');
var JobScheduler = mongoose.model('JobScheduler', jobSchedulerSchema, 'ALB_JobScheduler');
					...
					if (err) {
						...
						User.updateOne(myquery, newvalues, function(err, result) {
							if (err){
								...
								res.json({message: 'Password Reset', id:resetInfo.UserId , status: 1});
							}
						});
					}
				})
					...
					bcrypt.hash(userInfo.pwd, salt, function(err, hash) {
						if (err) {
							...
							newUser.save(function(err, User){
								if(err){
									if((err.errors.User_name!=undefined)&&(err.errors.User_name.kind==='unique')){
										res.json({message: 'User name already exists' , status: 1});
									}
									else if((err.errors.Email_Id!=undefined)&&(err.errors.Email_Id.kind==='unique')){
...
router.post('/api/addOrganization', function(req, res, next) {
	...
	newOrganization.save(function(err, Roles){
		if(err)
			...
			res.json({message: 'Roles added successfully' , status: 0});
	});  
	// var userInfo = req.body;
	// var startDate = moment(userInfo.startdate).format("YYYY-MM-DD HH:mm:ss");
	// var endDate = moment(userInfo.enddate).format("YYYY-MM-DD HH:mm:ss");
...
router.post('/api/addItemMaster', function(req, res, next) {
	...
	var startDate = moment(itemMasterInfo.startDate).format("YYYY-MM-DD HH:mm:ss");
		...
		if(err || response.length == 0)
			...
			newItemMaster.save(function(err, ItemMaster){
				 if(err)
					...
					res.json({message: 'ItemMaster added successfully' , status: 0});
			});
		}
	})
})
            ...
            res.json({message: 'User Not Authenticated' , status: 1});
				...
				newOrganization.save(function(err, Organization){
					 if(err)
						...
						res.json({message: 'Organizatioonal Details added successfully' , status: 0});
				});  
			}
	})
})
...
router.post('/api/addInventory', function(req, res, next) {
	...
	var startDate = moment(inventoryInfo.yearStartDate).format("YYYY-MM-DD HH:mm:ss");
	var endDate = moment(inventoryInfo.yearEndDate).format("YYYY-MM-DD HH:mm:ss");
	var PrStartDate = moment(inventoryInfo.pereiodStartDate).format("YYYY-MM-DD HH:mm:ss");
	var PrEndDate = moment(inventoryInfo.periodEndDate).format("YYYY-MM-DD HH:mm:ss");
		...
		if(err || response.length == 0)
			...
			newInventory.save(function(err, InventoryCalender){
				 if(err)
					...
					res.json({message: 'Inventory Calender added successfully' , status: 0});
			});  
		}
	})
})
		...
		if(err || response.length == 0)
			...
			newEmployee.save(function(err, Employee){
				 if(err)
					...
					res.json({message: 'Employee added successfully' , status: 0});
			});  
		}
	})
})
		...
		if(err || response.length == 0)
			...
			newReason.save(function(err, Reason){
				 if(err)
					...
					res.json({message: 'Reason added successfully' , status: 0});
			});
		}
	})
})
		...
		if(err || response.length == 0)
			...
			newAspJob.save(function(err, AspJob){
				 if(err)
					...
					res.json({message: 'AspJob added successfully' , status: 0});
			});
		}
	})
})
		...
		if(err || response.length == 0)
			...
			newPrdDataHr.save(function(err, PrdDataHr){
				 if(err)
					...
					res.json({message: 'PrdDataHr added successfully' , status: 0});
			});
		}
	})
})
...
router.post('/api/importUsers',multer({storage:store}).single('fileKey'), function(req, res, next) {
